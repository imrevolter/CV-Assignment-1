...
print("\nDiscussion:")
print("Lower thresholds on the gradient magnitude let more edges through, but a lot")
print("of them are just noise/texture (unwanted edges). Higher thresholds clean")
print("that up but start dropping faint, real edges too (missing edges). There's no")
print("single threshold that's perfect for both images.")
print("First-order (Sobel) edges are thick and respond a lot to texture, so they're")
print("noisy on the cameraman image (grass etc) but fine on coins.")
print("The plain Laplacian is very sensitive to noise since it's a second")
print("derivative, so it picks up a lot of spurious zero-crossings, especially on")
print("the textured image.")
print("LoG (gaussian blur before the laplacian) fixes a lot of that noise")
print("sensitivity and gives thin, well-localised edges, at the cost of rounding")
print("off very fine detail.")
print("Canny gives the cleanest, thinnest, most continuous edges thanks to non-max")
print("suppression + hysteresis, and is the best overall for the coins image with")
print("clean boundaries. On the cameraman image it still does reasonably well but")
print("can miss some very fine grass texture that a raw Sobel edge map would still")
print("show.")
print("Overall: Canny is the best default choice for the coins image (clean")
print("boundaries, needs continuity), while a lower-threshold Sobel or LoG is more")
print("useful than Canny if the goal is to catch fine texture detail.")

print("\nAll outputs saved in the Q4_outputs folder.")
